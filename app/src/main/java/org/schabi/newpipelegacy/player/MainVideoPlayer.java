...
public final class MainVideoPlayer extends AppCompatActivity
    ...
    protected void onResume() {
        ...
        if (playerState != null) {
            playerImpl.playQueue = playerState.getPlayQueue();
            playerImpl.setRepeatMode(playerState.getRepeatMode());
            playerImpl.setPlaybackParameters(playerState.getPlaybackSpeed(), playerState.getPlaybackPitch(), playerState.isPlaybackSkipSilence());
            ...
            playerImpl.initPlayback(playerState.getPlayQueue(), playerState.getRepeatMode(),
                    ...
                    playerState.isPlaybackSkipSilence(), playerState.wasPlaying(), false);
        }
    }
    ...
    private class VideoPlayerImpl extends VideoPlayer {
        ...
        @Override
        protected void setupSubtitleView(@NonNull SurfaceView view,
        ...
        public boolean isMuted() {
            if (mMediaPlayer == null || mMediaPlayer.isReleased()) return false;
        ...
        private void setInitialGestureValues() {
            if (mMediaPlayer != null && !mMediaPlayer.isReleased()) {
                final float currentVolumeNormalized = (float) mMediaPlayer.getVolume() / 100;
            ...
            return new PlayQueueItemTouchCallback() {
                ...
                public void onSwiped(int index) {
                    if (index != -1) {
                        ...
                        long progress = 0;
                        if (playQueue.getIndex() == index && mMediaPlayer.isPlaying()) {
                            progress = mMediaPlayer.getTime();
                            mMediaPlayer.stop();
                            canPlay = true;
                        }

                        playQueue.remove(index);
                        if(index != -1 && canPlay && (playQueue.size() > 0))
                            initPlayer(true);
                        else
                            onPlaybackShutdown();
    ...
    private class PlayerGestureListener extends GestureDetector.SimpleOnGestureListener implements View.OnTouchListener {
        ...
        // TODO
        //private final int maxVolume = playerImpl.getPlayer().getMaxVolume();
        ...
        public boolean onScroll(MotionEvent initialEvent, MotionEvent movingEvent, float distanceX, float distanceY) {
            ...
            if (isVolumeGestureEnabled && acceptVolumeArea) {
                ...
                playerImpl.getPlayer().setVolume(currentVolume);
